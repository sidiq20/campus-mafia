    ...
    function connect() {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'TerritoryAttacked') { title = 'Attack Detected'; body = `${data.territory_name} was hit!`; toast.error(title, { description: body }); window.dispatchEvent(new CustomEvent('territory-burst-ws', { detail: { eventType: 'TerritoryAttacked', territoryName: data.territory_name, factionName: data.attacker_faction } })); }
          if (data.type === 'TerritoryCaptured') { title = 'Territory Captured'; body = `${data.territory_name} was taken!`; toast.success(title, { description: body }); window.dispatchEvent(new CustomEvent('territory-burst-ws', { detail: { eventType: 'TerritoryCaptured', territoryName: data.territory_name, factionName: data.new_faction } })); }
